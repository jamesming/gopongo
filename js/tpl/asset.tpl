<li asset_id={{asset_id}}  class=' draggable ' >
	<div  youtube_id='{{youtube_id}}' class='play '   style='background:url({{youtube_thumb}}) no-repeat;background-position: 0px -45px;background-size: 282px;'  >&nbsp;
	</div>
	<div  class='overlay' >
		<span youtube_id='{{youtube_id}}' asset_id={{asset_id}} class='title play' >{{asset_name}}</span> |
		<span asset_id={{asset_id}} class='edit '  href='#edit_asset_modal_box' > Edit {{asset_id}}</span>
		<span asset_id={{asset_id}} class='delete ' > Delete {{asset_id}}</span>
		<span  class='dragHandle' >Drag</span>
	</div>
</li>